...
const Process = () => {
  return (
    <section id="proceso" className="relative py-16 sm:py-20 px-4 sm:px-6 border-y border-white/5 overflow-hidden" style={{ background: '#09090B' }}>
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-blue-500/[0.02] rounded-full blur-3xl pointer-events-none" />
          ...
          {steps.map((step, i) => {
            ...
            return (
              <Reveal key={i} animation="fade-up" delay={i * 100}>
                <div
                  className="group relative overflow-hidden bg-white/[0.02] border border-white/[0.06] p-6 rounded-2xl backdrop-blur-sm transition-[transform,box-shadow,border-color] duration-500 hover:-translate-y-2 hover:border-blue-500/20 hover:shadow-xl hover:shadow-blue-500/5 h-full flex flex-col"
                >
